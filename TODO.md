...
- [x] **Health / readiness dashboard:** `GET /health`, `GET /` never fetched. `lib/preflight.ts:157` probes lightweight `GET /health`. Add status dot + `ffmpegVersion/diskFreeHuman/queue{active,queued}` in `AdminHeader/JobsArea`.
- [x] **Storage / quota dashboard:** `GET /storage/stats` polled every 30 s via `useStorageStatsQuery` (`hooks/useStorageStats.ts` over `lib/storage.ts`). `StorageArea` shows quota bar (used of quota %, file counts, asset/artifact split) + Sweep button (`POST /api/storage/sweep` on-demand reconcile, live jobs untouched). Warns at ≥90% since new exports then fail with `507 QUOTA_EXCEEDED/DISK_FULL`.
- [x] **Upload session resume/abort UI:** `GET /api/upload/sessions` (new list endpoint, 10 s poll via `useUploadSessionsQuery` over `lib/upload-sessions.ts`) renders each open session in `UploadSessions` with resume progress bar, age/stale badge, per-row Abort (`DELETE /upload/:uploadId`, refcount-aware). Re-uploading the same file auto-resumes (`uploadFile` verifies remembered session via status `chunks[]` skip-set, skips sent chunks, "Resumed from N%" toast).
- [x] **Alternate output download/compare:** `JobEntry.alternateFile` rendered in `components/admin/JobRow.tsx` with Alt download (`GET /api/files/:id/download`, first web caller, saved with `-alt` suffix) + Alt compare (`exportHistory.openFileComparison` → source-vs-alternate dialog). Shared `downloadPlan` helper in `useAdminJobs` (was inline in `handleDownloadOne`).
- [x] **Full probe inspector:** `useExtendedVideoMetadataMutation` takes `{file,includeFrames,includePackets}` vars (shared `fetchVideoMetadata` fetch over `lib/upload-chunked`). Crop `Sidebar` Info section has a Deep-probe toggle (`true/true`) + `ProbeInspector` dialog (Shadcn Tabs: Streams/Format/Frames/Packets/Raw JSON, paginated tables) over the already-returned `ffprobeReport`. Backend `splitPacketsAndFrames` (`utils/metadata.ts`) splits ffprobe ≥6's merged `packets_and_frames` back into `frames[]`/`packets[]` (+ unit test).
- [x] **`x-upload-id` reuse for audio:** `routes/audio.ts:resolveInput` supports it, and now the web uses it: `lib/audio-upload.ts` (`audioUpload.ensureTransport/postJson/postBlob`) caches one completed chunked upload per file (>256 MB, status-validated, in-flight dedupe, evict + retry once on FILE_REQUIRED) shared by `useAudioAnalysis`, `useAudioPreview` (one transport across all per-track pulls), `AudioControls` extract, `Sidebar` audio-extract `run`, and `exportHistory.retryAudioExtract` — header-only POSTs, no double-send; small files keep direct FormData.
- [x] **`audioTracks[]` on subtitles + bulk:** subtitles endpoint ignored them (`buildMobileSubtitlesArgs` hardcoded `-map 0:a[?]`) — backend now takes `audioTracks[]` (per-track `-map 0:a:<idx>?`, legacy default when omitted; + unit tests) and the route forwards `settings.audioTracks`. Web: `useSubtitleExport` forwards `audioStore` tracks (AudioControls Include switches = picker) inside the v1 `mobile-subtitles` envelope the route requires (bare v0 was rejected as kind `"mobile"` — that bug is fixed as part of this); `BulkExpandedView` has a per-video picker (on-demand `useAudioAnalysis` probe → `BulkItem.audioTracks[]`) forwarded per item by `useBulkExport`.
- [x] **`customFFmpegArgs` everywhere:** shared `CustomArgsCollapsible` (`components/editor/CustomArgsCollapsible.tsx`, Shadcn Collapsible + Textarea — crop `Sidebar` refactored onto it) in all five export forms: mobile `PreviewPanel` (state in `useMobileEditor` → `useMobileExport`), cut `CutSettingsSidebar` (shared `cutStore` field → `useCutExport`), subtitles `SubtitleSettingsPanel` card (state in `useSubtitleExport` via `useSubtitleEditor`), bulk `BulkSettingsPanel` (global value from `useBulkEditorState` → per-item settings in `useBulkExport`). Backend `parseCustomArgs` denylist unchanged — managed flags fail fast with `400 CUSTOM_ARGS_INVALID`.
- [x] **`POST /transcode/clear` alias:** dead — deleted. Zero callers (web clear-all uses `DELETE /transcode/jobs[?status=]`); the alias duplicated the delete loop minus the legacy `/tmp` sweep (`swept`). Removed the handler (`routes/video.ts`) + alias bullet and stale `POST /clear` mentions in api README/AGENTS. Live: `POST` → 404, canonical `DELETE` intact.
...
- [x] `components/editor/Sidebar.tsx` **1123 lines** — was merging `{...source,...crop,...cut}` (any slice re-rendered whole form). Split → thin `Sidebar.tsx` composer (`SidebarToggle` + cards) + one file per card in `crop/` (`SidebarFileButton/InfoCard/CropCard/SpeedCard/ExportCard/PreflightSummary`, each subscribing only to its slice) + `crop/useCropExport.ts` (presets/preflight/queue/export, crop rect read imperatively at click time).
