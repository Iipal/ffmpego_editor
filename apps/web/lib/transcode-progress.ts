...
//
// Two consumers:
// - `subscribeTranscodeProgress` — push-style; the export queue drives the
//   store directly (no blocked page), so it owns the EventSource lifecycle.
// - `awaitTranscodeCompletion` — legacy await-style built on top of the
//   subscriber so error handling (log tails, cancel, reconnect) stays in one
//   place.
//
...
const RECONNECT_DELAYS_MS = [2000, 2000, 2000];
...
export function subscribeTranscodeProgress(
  ...
  const open = () => {
    ...
    source.onerror = () => {
      ...
      if (retry < RECONNECT_DELAYS_MS.length) {
...
export function awaitTranscodeCompletion(
  ...
  return new Promise<void>((resolve, reject) => {
    const dispose = subscribeTranscodeProgress(progressUrl, {
