...
export function enforceZoneAspect(
  z: CropZone,
  mode: MobileLayoutMode,
  split: number,
  sourceAR = 16 / 9,
...
export function resizeZoneAspectLocked(
  start: CropZone,
  handle: string,
  dx: number,
  dy: number,
  ...
  if (handle === "se") {
    ...
    let h = w / R;
    w = clamp(w, MIN_ZONE, 1 - anchorX);
    h = clamp(h, MIN_ZONE, 1 - anchorY);
  ...
  if (handle === "nw") {
    ...
    let w = sw * clampedScale;
    let h = w / R;
    w = clamp(w, MIN_ZONE, anchorX);
    h = clamp(h, MIN_ZONE, anchorY);
    let x = anchorX - w;
    let y = anchorY - h;
    if (x < 0) {
      x = 0;
...
export function autoSuggest(mode: MobileLayoutMode, split = 0.5): MobileLayout {
  ...
  return {
    ...
    mode: "stacked",
    splitRatio: split,
    background: { type: "blur", intensity: 14 },
    zones: [
      {
        id: "zone-1",
        x: 0.68,
...
export function loadPref(): MobileLayout | null {
  try {
