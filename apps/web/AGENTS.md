...
| `lib/`        | `api-client.ts` (`APIClient` service: `apiClient.url/get/post/formPost`), `upload-chunked.ts`, `export-queue.ts` (`ExportQueue` service: `exportQueue.enqueue/cancel/dismiss`), `transcode-progress.ts` (`subscribeTranscodeProgress`), `transcode-jobs.ts` (`cancelTranscodeJob`, envelope errors), `validate-settings.ts`, `export-presets.ts`, `preflight.ts`, `export-history.ts` (`ExportHistory` service: `exportHistory.renameJob/retryEntry/openComparison/retryAudioExtract`), `video-file.ts`, `mobile-layout.ts`, `format-time.ts`, `save-blob-file.ts` |                |                      |                                  |                                                                                      |                                                          |                                                                  |                    |           |                   |
